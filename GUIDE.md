...
Target custom domain: **careychou.tech** (confirm availability at purchase — swap if taken)
...
1. Buy `careychou.tech` at https://porkbun.com or https://namecheap.com.
2. In Cloudflare dashboard → **Add a site** → enter `careychou.tech` → choose **Free** plan.
...
2. Click **Set up a custom domain** → enter `careychou.tech` (and `www.careychou.tech`).
3. Cloudflare adds the DNS records automatically (if the zone is on Cloudflare).
4. HTTPS cert is issued automatically — done. Your site is live at `https://careychou.tech`.
