...
Project name: **careychou-cintegral** → free URL will be **https://careychou-cintegral.pages.dev**
Target custom domain: **careychou-cintegral.com** (confirm availability at purchase — swap if taken)
...
In your terminal:
```bash
cd ~/Documents/careychou-cintegral
npx wrangler login
```
...
`https://careychou-cintegral.pages.dev`. Every future publish is the same one command.
...
2. Search `careychou-cintegral` → pick an available TLD (`.com`, `.dev`, etc.) → checkout.
...
1. Buy `careychou-cintegral.com` at https://porkbun.com or https://namecheap.com.
2. In Cloudflare dashboard → **Add a site** → enter `careychou-cintegral.com` → choose **Free** plan.
...
1. Dashboard → **Workers & Pages → careychou-cintegral → Custom domains**.
2. Click **Set up a custom domain** → enter `careychou-cintegral.com` (and `www.careychou-cintegral.com`).
3. Cloudflare adds the DNS records automatically (if the zone is on Cloudflare).
4. HTTPS cert is issued automatically — done. Your site is live at `https://careychou-cintegral.com`.
